...
from jaxtyping import ArrayLike, Real, Bool, PyTree, Array, Int
...
from ..analysis.wba import wba_weights
...
class PunctureData(NamedTuple):
    ...
    tp: Real[ArrayLike, "Npunc"]
        Times of punctures
    yp: PyTree of Real[ArrayLike, "Npunc"]
        States at punctures
    sign: Int[ArrayLike, "Npunc"]
        Sign of the puncture (+1 or -1)
    """
    tp: Real[np.ndarray, "Npunc"]
    yp: PyTree[Real[np.ndarray, "Npunc"]]
    sign: Int[np.ndarray, "Npunc"]
    ...
    def empty_list_like(cls, state: PusherState) -> list[PunctureData]:
        ...
        return list(cls(
            ...
            sign = np.zeros((0,), dtype=int)
        ) for _ in range(state.r.shape[0]))


def compute_punctures(ts: Real[ArrayLike, "Nt"], ys: PyTree[Real[ArrayLike, "Nt *shape"]], fpunc: Real[ArrayLike, "Nt N"], condpunc: Bool[ArrayLike, "Nt"] | None = None, period: float =-1) -> list[PunctureData]:
    """
    ...
    for k in range(nump):
        ...
        if period < 0:
            ...
            jumps = pjumps | njumps
            signs = np.where(pjumps, 1, 0) - np.where(njumps, 1, 0)
        else:
        ...
        for j in range(ind_puncs.shape[0]):
            ...
            tp[j] = (1-ffrac)*ts[tind] + ffrac*ts[tind+1]
            yp[j] = jax.tree.map(lambda z: (1-ffrac)*z[tind,k] + ffrac*z[tind+1,k], ys)

        # Compute tree transposes
        if len(yp) > 0:
            yp = jax.tree.map(lambda *xs: np.stack(xs), *yp)
        else:
            yp = jax.tree.map(lambda x: np.zeros((0,)), ys)
        puncs[k] = PunctureData(tp, yp, sign)
